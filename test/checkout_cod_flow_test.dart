...
void main() {
  group('Cash on Delivery (COD) Flow & Product Eligibility Tests', () {
    test(
      'ProductModel parses isCodAllowed correctly from different JSON keys',
      () {
    ...
    test(
      'Single product cart with COD disabled displays accurate product message',
      () {
        ...
        final cart = [CartItemModel(id: 'c1', product: pDisabled, quantity: 1)];

        final isCodAllowed = !cart.any(
          (item) => item.product.isCodAllowed == false,
        );
        ...
        expect(
          ...
          equals('Cash on Delivery is not available for this product.'),
        );
      },
    );

    test(
      'Mixed cart (COD enabled + COD disabled) displays mixed cart rule message',
      () {
    ...
    test(
      'OrderModel parses payment method correctly from nested payment object',
      () {
        final orderJson = {
          'id': 101,
    ...
    test(
      'Canonical payment method mapping ensures COD is passed correctly',
      () {
    ...
    test(
      'Button label switches dynamically between PLACE ORDER and PAY NOW',
      () {
        String getButtonText(String selectedPayment) {
          return (selectedPayment == 'Razorpay' ||
    ...
    test(
      'DioException 400 error message extraction provides clear explanation',
      () {
