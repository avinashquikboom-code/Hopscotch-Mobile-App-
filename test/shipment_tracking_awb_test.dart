...
void main() {
  group('OrderModel Shipment & AWB Deserialization Tests', () {
    test('Preserves AWB as string with leading zeros and special characters without converting to int', () {
      final json = {
        'id': 'ord-101',
      ...
      expect(order.trackingUrl, 'https://www.delhivery.com/track/package/000987654321');
    });
    ...
    test('Correctly extracts AWB and courier from nested shipment object', () {
      final json = {
        ...
        'shipment': {
          'awb': 'BLU-998877',
          'courier': 'Bluedart',
          'trackingUrl': 'https://www.bluedart.com/tracking?handler=tnt&action=custtrack&trackid=BLU-998877',
        },
        'items': [],
      };
      ...
      expect(order.trackingUrl, 'https://www.bluedart.com/tracking?handler=tnt&action=custtrack&trackid=BLU-998877');
    });

    test('Gracefully handles unshipped order without fake tracking numbers', () {
      final json = {
        'id': 'ord-103',
  ...
  group('InvoiceGenerator AWB & Courier Integration Test', () {
    test('Generates PDF invoice containing AWB Number and Courier Partner', () async {
      ...
      final pdfBytes = await InvoiceGenerator.buildInvoicePdfBytes(order: sampleOrder);
      expect(pdfBytes, isNotNull);
      expect(pdfBytes.length, greaterThan(1000));

      final artifactPath = '/Users/avinashsanjaymagar/.gemini/antigravity-ide/brain/3823807e-4a6b-46a8-9515-afb91d0c1a58/fashion_city_invoice_with_awb.pdf';
